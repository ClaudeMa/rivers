...
CREATE TYPE reltype AS ENUM ('river',  'canal', 'stream', 'boundary');

DROP TABLE IF EXISTS relations CASCADE;
CREATE TABLE relations (osm_id BIGINT PRIMARY KEY, name VARCHAR NOT NULL, t reltype, sandre VARCHAR DEFAULT NULL);
...
CREATE INDEX waysinrelboundarygeom_relid_seq on waysinrel (rel_id);
CREATE INDEX waysinrel_wayid_seq on waysinrel (way_id);

--- ways table
DROP TYPE IF EXISTS waytype CASCADE;
CREATE TYPE waytype AS ENUM ('bridge','lock','tunnel');

DROP TABLE IF EXISTS ways CASCADE;
CREATE TABLE ways (osm_id BIGINT PRIMARY KEY, name VARCHAR NOT NULL, t waytype DEFAULT NULL, cemt VARCHAR DEFAULT NULL, lock_name VARCHAR DEFAULT NULL, lock_ref VARCHAR DEFAULT NULL, lock_height VARCHAR DEFAULT NULL, motorboat VARCHAR DEFAULT NULL);
...
CREATE TABLE nodesinway (id SERIAL PRIMARY KEY, way_id BIGINT NOT NULL, node_id BIGINT NOT NULL);
...
CREATE TABLE nodes (osm_id BIGINT PRIMARY KEY);
